...
`binary` mode uses separate schemas (`pwn-agent.binary-analysis.v1`, `pwn-agent.binary-plan.v2`, `pwn-agent.binary-verify.v1`, `pwn-agent.binary-patch-validation.v1`) so source-audit structures are not overloaded.
